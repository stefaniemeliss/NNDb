...
#subjects=($(ls -d {12_years_a_slave,the_prestige}/[0-9][0-9][0-9][0-9][0-9][0-9][A-Z][A-Z])) #subject ID 6 numbers + 2 letters
#subjects=($(ls -d {little_miss_sunshine,the_usual_suspects}/[0-9][0-9][0-9][0-9][0-9][0-9][A-Z][A-Z])) #subject ID 6 numbers + 2 letters
	...
	echo ""
		...
		# loop over subjects+1
				...
				ISC_prefix=${ISC_p//$find/$replace}
					...
					# check whether there are no more than 10 3dTcorrelate are running
					# if [[ $num_running -lt 3 ]]; then
						...
						# nohup 3dTcorrelate -pearson -polort -1 -Fisher -automask -prefix $ISC_prefix $s1_file[0..5469] $s2_file[0..5469]  &> nohup_"$ISC_prefix".out &
						3dTcorrelate -pearson -polort -1 -Fisher -automask -prefix $ISC_prefix $s1_file[0..5469] $s2_file[0..5469]
            #                    	else

						#while [[ $num_running -ge 3 ]]; do

							# wait
							# echo "Waiting 5 seconds"
							# sleep 5

							# check again how many processes are running
							# num_running=`ps -aux | grep "3dTcorrelate" | grep -v grep | wc -l`
...
printf "3dISC -prefix ISC_across_movies -jobs 12  -model 'grp+(1|Subj1)+(1|Subj2)' -qVars grp -gltCode ave '1 0' -gltCode within_vs_between '0 2' -gltCode within '1 1' -gltCode between '1 -1' -mask ./masks/anatomical_mask_no_wm_ventricle.nii.gz -dataTable @DataTable_ISC_across_movies.txt " > ./run_3dISC_across_movies

# make file executable
chmod +x run_3dISC_across_movies

# calculate ISC with LME-CRE
nohup ./run_3dISC_across_movies &> ./output_3dISC_across_movies.txt &
